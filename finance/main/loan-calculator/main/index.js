...
if (window[SINGLETON_KEY]) {
  ...
  console.error(
    'Collision detected: Multiple instances of Loan Calculator found.'
  );
} else {
  window[SINGLETON_KEY] = true;

  const loanAmountInput = fragmentElement.querySelector(
    `#loanAmount-${fragmentEntryLinkNamespace}`
  );
  const loanTermInput = fragmentElement.querySelector(
    `#loanTerm-${fragmentEntryLinkNamespace}`
  );
  const amountValue = fragmentElement.querySelector(
    `#amountValue-${fragmentEntryLinkNamespace}`
  );
  const termValue = fragmentElement.querySelector(
    `#termValue-${fragmentEntryLinkNamespace}`
  );
  const totalPaymentText = fragmentElement.querySelector(
    `#totalPayment-${fragmentEntryLinkNamespace}`
  );
  const termMonthsText = fragmentElement.querySelector(
    `#termMonths-${fragmentEntryLinkNamespace}`
  );
  const interestRateText = fragmentElement.querySelector(
    `#interestRate-${fragmentEntryLinkNamespace}`
  );
