...
[/#if]
		...
		<input aria-describedby="${fragmentEntryLinkNamespace}-range-input-help-text" aria-labelledby="${fragmentEntryLinkNamespace}-range-input-label [#if input.errorMessage?has_content]${fragmentEntryLinkNamespace}-range-input-error-message[/#if]
" class="form-control" data-validation-message-text="${languageUtil.get(locale, "enter-a-valid-value.-the-value-has-too-many-decimals")}" id="${fragmentEntryLinkNamespace}-range-input" [#if (configuration.customRange && configuration.max??)]max="${configuration.max!}"
[#elseif input.attributes.max??]max="${input.attributes.max}"[/#if] [#if (configuration.customRange && configuration.min??)]min="${configuration.min!}"
[#elseif input.attributes.min??]min="${input.attributes.min}"[/#if] name="${input.name}" [#if readOnly]readonly[/#if] ${input.required?then('required', '')}
[#if (configuration.customRange && configuration.step??)]step="${configuration.step!}"
[#elseif input.attributes.dataType?? && input.attributes.dataType="decimal" && input.attributes.step??]step="${input.attributes.step}"[/#if] placeholder="${configuration.placeholder!}" type="range" [#if input.value??]value="${input.value}"[/#if] />
