...
" for="${fragmentEntryLinkNamespace}-text-input">${htmlUtil.escape(input.label)}
...
[/#if]
			...
			<div class="text-input confirmation">
				<label class="
[#if !input.showLabel || !input.label?has_content]sr-only[/#if]
" for="${fragmentEntryLinkNamespace}-text-confirmation-input">${htmlUtil.escape(configuration.label + ' ' + configuration.convertLabelToLowercase?then(input.label?lower_case, input.label))}
[#if input.required][@clay["icon"] className="reference-mark" symbol="asterisk" /]
[/#if]
</label>
