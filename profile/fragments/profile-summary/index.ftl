...
<div class="profile-wrapper">
  <div class="profile-summary">
    <div class="profile-photo">
      <img
        src="${profileImageUrl}"
        alt="Profile picture of ${profileUser.getFirstName()} ${profileUser.getLastName()}"
      />
    </div>
    <div class="profile-header">
      <h1>${profileUser.getFirstName()} ${profileUser.getLastName()}</h1>
      <h3>${profileUser.getJobTitle()}</h3>
    </div>
  </div>
  <div class="profile-detail">
    <div class="row">
      ...
      <div class="cell">
        ${profileUser.getFirstName()!""}${(profileUser.getMiddleName()?has_content)?string('
        ' + profileUser.getMiddleName(),'')} ${profileUser.getLastName()!""}
      </div>
    </div>
    <div class="row">
      <div class="cell">Email</div>
      <div class="cell">${profileUser.getEmailAddress()!""}</div>
      <div class="cell label-${displayType}">
        ${profileUser.emailAddressVerified?string('Verified', 'Unverified')}
      </div>
    </div>
    <div class="row">
      <div class="cell">LinkedIn</div>
      <div class="cell">
        ${profileUser.getExpandoBridge().getAttribute("LinkedIn")!""}
      </div>
      <div class="cell">
        <button
          class="btn-unstyled copy-to-clipboard"
    ...
    <div class="row">
      ...
      <div class="cell">
        ${themeDisplay.getPortalURL() + "/web/" + screenName + "/profile"}
      </div>
      <div class="cell">
        <button
          class="btn-unstyled copy-to-clipboard"
          aria-label="Copy profile page URL to clipboard"
        >
          [@clay["icon"] symbol="share" /]
        </button>
      </div>
  ...
  <div id="pdf-wrapper-${screenName?replace('.','-')}" class="pdf-wrapper">
    ...
    <div class="profile-detail">
      ...
      <div class="row">
        <div class="cell">Profile</div>
        <div class="cell">
          ${themeDisplay.getPortalURL() + "/web/" + screenName}
        </div>
      </div>
    </div>
  </div>
