...
- `fragmentElement`: The root DOM element of the fragment instance. Use for all
  internal selectors.
- `configuration`: An object containing user-set values from
  `configuration.json`.
- `fragmentEntryLinkNamespace`: A unique identifier for the fragment instance on
  the page. Use for scoping IDs and classes.
...
- **Null Safety**: Always provide default values:
  `${configuration.title!'Default'}`.
- **CSS Variable Bridge**: Map configuration values to CSS variables in the HTML
