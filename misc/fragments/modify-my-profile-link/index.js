...
if (enableMenuText) {
	menuTextFunc = () => {
		...
		const _navbarMenu = document.querySelector('div.dropdown-menu-personal-menu');
		if (_navbarMenu) {
			const _a = queryInnerText(_navbarMenu, 'a', menuText);	
			if (_a) {
				var link = _a.href;
...
if (runMenuTextOnload) {
	document.addEventListener('DOMContentLoaded', () => {
		if (runMenuTextOnload && menuTextFunc) {
			...
			const _navbarMenu = document.querySelector('div.personal-menu-dropdown');
			if (_navbarMenu) {
				_navbarMenu.addEventListener('click', menuTextFunc);
			}
		}
	});
}
