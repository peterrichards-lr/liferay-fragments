...
The local `.git/hooks/pre-commit` hook is generated by
`scripts/install-git-hooks.js` and runs automatically on every `git commit`,
enforcing the following checks in order:

1. **Workspace cleanliness check** (`scripts/check-cleanliness.js`) — aborts
   the commit if the workspace is not in an expected state (Issue #66).
2. **`prettier`** — formats staged HTML, CSS, JS, JSON, and MD files in place.
3. **Dependency sync** (`scripts/initialize-build-config.js`) — ensures all
   collection `fragment-build.json` files are up to date.
4. **Visual gallery generation** (`scripts/generate-gallery.js`) — regenerates
   `docs/gallery.md` from the current fragment set.
5. **Fragment Quality Gate** (`npm run lint`) — enforces schema validation,
   localization coverage, field integrity, and theme fidelity.
6. **Secret detection** (`scripts/detect-secrets.js pre-commit`) — scans
   staged files for hardcoded credentials or secrets. This is a custom
   in-repo script, not the third-party `gitleaks` tool — it only reuses the
   `.gitleaksignore` filename convention for allowlisting.
