...
<style>
  ...
  .fragment-root .hamburger-zone-inner {
    ...
    display:flex; flex-direction:row; transition:ease all .5s; max-width:100%; flex-wrap: nowrap;
    align-items:${configuration.dropzoneAlignItems}; gap:${configuration.dropzoneGap};
  }
  ...
  .is-menu-view .logo-zone[data-always-display="false"] {
    display: block;
  }
  ...
  @media (max-width:${configuration.landscapePhoneBreakpoint}) {
    ...
    .fragment-root .hamburger-zone-inner .dropzone-right {
      visibility:visible; overflow:visible; opacity:1; transition:all .5s ease; min-width:100%;
    }
