  ...
  if (layoutMode === "view") {
    ...
    function convertToPixels(
      ...
      switch (unit) {
        ...
        case "%":
          const parent =
            contextElement.parentElement || document.documentElement;
    ...
    if (debugEnabled) debugWithContext("getSkuUrl", getSkuUrl);

    const applyBadgeOverlayAction = () => {
      ...
      const determineBadgeType = (product) => {
        ...
        const hasLimitedStock =
          stockQty <= parseFloat(limitedStockLevelThreshold);
        const hasDiscount = promoPrice < regularPrice;
        const isNew =
          freshnessDate && newDate && new Date(newDate) >= freshnessDate;
      ...
      const getCssVariable = (varName, root = document.documentElement) => {
        return getComputedStyle(root).getPropertyValue(varName).trim() || null;
      };
      ...
      const generateBadgeImage = (badgeConfig) => {
        if (debugEnabled) debugWithContext("badgeConfig", badgeConfig);
        ...
        const drawDefaultBadge = () => {
          ...
          drawBadgeShape(
            ...
            0,
            0,
            canvas.width - shadowBlur,
            canvas.height - shadowBlur,
          ...
          const startY =
            (canvas.height - shadowBlur) / 2 -
            (lineHeight * (lines.length - 1)) / 2;

          lines.forEach((line, i) => {
            ctx.fillText(
              line,
              (canvas.width - shadowBlur) / 2,
              startY + i * lineHeight,
            );
          });
        };

        badgeConfig.styleType === "corner-ribbon"
          ? drawCornerRibbon()
          : drawDefaultBadge();
      ...
      const applyOverlay = (productDiv, product, badgeType) => {
        const determineShape = (shape) =>
          shape !== "corner-ribbon" ? shape : undefined;
        const determineStyleType = (config) =>
          config.shape !== "corner-ribbon" ? "normal" : "corner-ribbon";
        const determineBorderRadius = (config) => {
          switch (config.shape) {
            case "pill":
              return `${Math.max(config.width, config.height) / 2}px`;
            case "rectangle":
              return "10px";
            default:
              return "0";
      ...
      Liferay.Util.fetch(getSkuUrl)
        ...
        .then((json) => {
          ...
          if (product && badgeType !== "none") {
            const productCard =
              fragmentElement.querySelector("div.product-card");
            if (productCard) {
    ...
    if (allPortletsReady) {
      Liferay.on("allPortletsReady", () => {
        setTimeout(() => applyBadgeOverlayAction(), initializeDelay);
      });
    } else {
      setTimeout(() => applyBadgeOverlayAction(), initializeDelay);
    }
