const initGallery = async (isEditMode) => {
  ...
  if (!Liferay.Fragment.Commons.isValidIdentifier(objectERC)) {
    ...
    try {
      ...
      if (!definition) throw new Error('Definition fetch failed.');

      const pageSize = isEditMode ? 4 : 20;
      const dataRes = await Liferay.Util.fetch(
        `${apiPath}/?pageSize=${pageSize}`
      );
      if (!dataRes.ok) {
      ...
      if (items.length === 0) {
        if (grid) {
          Liferay.Fragment.Commons.renderEmptyState(grid, {
            ...
            description: `The ${definition.name} object currently has no data to display in the gallery.`,
          });
        }
      } else {
