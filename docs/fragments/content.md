...
This collection includes fragments for displaying content from Liferay structures, with a focus on Service-oriented displays.
...
A visually appealing card designed to display information from a Service content structure. It supports dynamic background colors, customizable icons, and optional debug logging.

## Service Icon

Similar to the Service Card, but focuses on an icon-centric display. It can pull icon information directly from the mapped content structure or use a default symbol defined in the configuration.

## Service Link Button

A dynamic button that changes its label and behavior based on the user's authentication status.

- **Signed In**: Displays "Go to Service" and links to the service URL.
- **Guest**: Displays "Login for Service" and redirects the user to the login page.
