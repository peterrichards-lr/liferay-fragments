...
- **Site-Scoped Discovery**: Automatically resolves the correct API path for both Company and Site-scoped Objects.

## Configuration

- **Object ERC**: The External Reference Code of the target Object definition (defaults to `AUDIT_ENTRY`).
...
- **Audit Action Prefix/Suffix**: Configurable text to wrap around the button label for the audit log.
...
- **`validation.js`**: Uses `isValidIdentifier` for robust configuration checking.
