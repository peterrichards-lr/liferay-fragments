...
- **Deployment Scoping:** To ensure fragments are visible and editable in the
  test environment, the script auto-deploys them specifically to the **Guest**
  site (`companyWebId: liferay.com`, `groupKey: Guest`). Deploying fragments
  globally (`*`) can cause them to appear as empty collections in the site-level
  Fragments UI.
