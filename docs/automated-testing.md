...
2. **Environment Provisioning:** Uses LDM to spin up a clean Liferay instance
   ...
   **Sidecar** architecture (bundling Elasticsearch and the DB into the same
   pod), defaults to the **Hypersonic (HSQLDB)** database, and communicates over
   **HTTP** (port 8080).
...
- **Tag Prefixing:** If the provided tag only contains the year and quarter
  (e.g. `2026.q1`), the script automatically uses the `--tag-prefix` flag to
  fetch the latest available update.
- **Port Conflict Resolution:** The script checks if port `8080` is busy. If it
  is, it automatically attempts to use port `8090` and dynamically updates the
  Playwright `BASE_URL` to ensure the tests can proceed without manual
  intervention.
...
If the Playwright tests fail, you can review the standard Playwright HTML report
generated in `e2e-tests/playwright-report/` or review the raw logs in
`e2e-tests/playwright_output.log`. These folders are excluded from Git.
