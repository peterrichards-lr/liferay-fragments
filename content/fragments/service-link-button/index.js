...
const debug = (label, ...args) => {
  if (debugEnabled) console.debug(`[Service Link Button] ${label}`, ...args);
};
...
const titleEl = fragmentElement.querySelector('span.config-title');
const title = titleEl?.textContent.trim();

const resetFragment = (showFragmentContent = true) => {
  if (showFragmentContent) {
    const serviceLinkButtonContent = fragmentElement.querySelector('div.service-link-button___content');
    serviceLinkButtonContent.classList.replace('d-none', 'd-block');
  }
...
if (title) {
  ...
  const convertValue = (rawData, dataType) => {
    ...
    switch (dataType) {
      ...
      case 'structuredContent': return rawData?.id;
    ...
    fields.forEach(field => {
      ...
      if (!dataType && /^Fieldset/i.test(name)) {
        ...
        const rawData =
          ...
          field.contentFieldValue?.geo ??
          field.contentFieldValue?.structuredContentLink;
  ...
  const errorHandler = (err) => {
    ...
    if (layoutMode !== 'view' || !(Liferay?.Util?.openToast)) {
      const heading = err.type === 'unexpected' ? 'Unexepcted exception' : 'Configure Your Service Link Button';
  ...
  const getPageUrls = (structuredContentId) => {
    const headlessPrefix = '/o/headless-delivery/v1.0';

    return Liferay.Util.fetch(`${headlessPrefix}/structured-contents/${structuredContentId}?fields=id%2CcontentFields&flatten=true`)
      .then(response => {
        if (!response.ok) {
        ...
        return collectFields(contentItem?.contentFields);
      });
  }
      ...
      .then(async data => {
        ...
        if (layoutMode === 'view') {
          if (items && items.length === 1) {
            ...
            getPageUrls(contentItem.serviceLink)
              .then(async service => {
                let pageUrl = getPageUrlBySiteId(service, useDefaultPageUrl, defaultPageUrl, destinationSiteId);
                debug(`The pageUrl for ${destinationSiteId} is ${pageUrl}`);
