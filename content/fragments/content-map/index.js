...
const loadCategoryStyles = async (taxonomyVocabularyId) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) throw { type: "data", message: "The vocabulary is not found" };
    ...
    json.items.forEach(cat => {
      ...
      if (!props) {
        throw { type: "data", message: "The vocabulary does not contain any properties." };
      }
      if (props.icon) {
        categoryStyles[cat.name] = {
          icon: props.icon,
          color: (props?.color[0] === '#' ? props.color : `#${props.color}` || "#0b5ffff"),
    ...
    if (!categoryStyles) {
      throw { type: "data", message: "The vocabulary does not contain any icon properties." };
    }
  ...
  Liferay.Util.fetch(headlessCallUrl)
    ...
    .then(async data => {
      ...
      const addMarkers = async () => {
        ...
        const categoryStyles = useCustomMarkers ? await loadCategoryStyles(vocabularyId) : null;
        ...
        for (const item of pois) {
          ...
          const availableStyles = categoryStyles ? cats
    ...
    .catch((err) => {
      if (err.name === 'InvalidValueError' && err.message.indexOf('gmp-advanced-marker') > -1) {
        // Loading issue, so reload
        location.reload();
      } else {
        errorHandler(err);
      }
